    ...
    <ul class="nav nav-tabs mb-2">
        ...
        @foreach($object->template->modules as $module)
            @if (in_array($module->pivot->placeholder, $object->template->placeholders))
                <li class="nav-item">
                    <a href="{{ route('admin.products.edit-module', ['pk' => $object->pk, 'placeholderId' => $module->pivot->id]) }}" class="nav-link {{ isset($placeholderId) && $module->pivot->id == $placeholderId ? 'active' : '' }}">{{ $module->title }} ({{ $module->pivot->title ?? $module->pivot->placeholder }})</a>
                </li>
            @endif
        @endforeach
    </ul>
